#!/usr/bin/env python3
#
# Licensed to the Apache Software Foundation (ASF) under one
# or more contributor license agreements.  See the NOTICE file
# distributed with this work for additional information
# regarding copyright ownership.  The ASF licenses this file
# to you under the Apache License, Version 2.0 (the
# "License"); you may not use this file except in compliance
# with the License.  You may obtain a copy of the License at
#
#   http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing,
# software distributed under the License is distributed on an
# "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
# KIND, either express or implied.  See the License for the
# specific language governing permissions and limitations
# under the License.
#

from itertools import chain
import argparse
import json
import pygal


def create_charts(out_dir, test_results):
    # Group results for same workload
    workload_results = {}
    for test_result in test_results:
        result = json.load(open(test_result))
        if not result["workload"] in workload_results:
            workload_results[result["workload"]] = []
        workload_results[result["workload"]].append(result)

    for workload, results in workload_results.items():
        print("Generating charts for", workload)
        workload = workload.replace("/", "-")

        create_chart(
            out_dir,
            workload,
            "Publish latency 99pct",
            y_label="Latency (ms)",
            time_series=[
                (x["driver"], x["publishLatency99pct"]) for x in results
            ],
        )

        create_chart(
            out_dir,
            workload,
            "Publish rate",
            y_label="Rate (msg/s)",
            time_series=[(x["driver"], x["publishRate"]) for x in results],
        )

        create_chart(
            out_dir,
            workload,
            "Consume rate",
            y_label="Rate (msg/s)",
            time_series=[(x["driver"], x["consumeRate"]) for x in results],
        )

        create_chart(
            out_dir,
            workload,
            "endToEndLatencyAvg",
            y_label="LatencyAvg msec",
            time_series=[
                (x["driver"], x["endToEndLatencyAvg"]) for x in results
            ],
        )

        create_quantile_chart(
            out_dir,
            workload,
            "Publish latency quantiles",
            y_label="Latency (ms)",
            time_series=[
                (x["driver"], x["aggregatedPublishLatencyQuantiles"])
                for x in results
            ],
        )


def create_chart(out_dir, workload, title, y_label, time_series):
    chart = pygal.XY(
        dots_size=0.3,
        legend_at_bottom=True,
    )
    chart.title = title

    chart.human_readable = True
    chart.y_title = y_label
    chart.x_title = "Time (seconds)"
    # line_chart.x_labels = [str(10 * x) for x in range(len(time_series[0][1]))]

    for label, values in time_series:
        chart.add(label, [(10 * x, y) for x, y in enumerate(values)])

    chart.range = (0, max(chain(*[l for (x, l) in time_series])) * 1.20)
    chart.render_to_file(f"{out_dir}/{workload}-{title}.svg")


def create_quantile_chart(out_dir, workload, title, y_label, time_series):
    import math

    chart = pygal.XY(  # style=pygal.style.LightColorizedStyle,
        # fill=True,
        legend_at_bottom=True,
        x_value_formatter=lambda x: "{} %".format(100.0 - (100.0 / (10**x))),
        show_dots=True,
        dots_size=0.3,
        show_x_guides=True,
    )
    chart.title = title
    # chart.stroke = False

    chart.human_readable = True
    chart.y_title = y_label
    chart.x_title = "Percentile"
    chart.x_labels = [1, 2, 3, 4, 5]

    for label, values in time_series:
        values = sorted((float(x), y) for x, y in values.items())
        xy_values = [
            (math.log10(100 / (100 - x)), y) for x, y in values if x <= 99.999
        ]
        chart.add(label, xy_values)

    chart.render_to_file(f"{out_dir}/{workload}-{title}.svg")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create charts")
    parser.add_argument(
        "-o",
        "--out",
        default=".",
        help="Out directory for charts",
    )
    parser.add_argument(
        "-r",
        "--results",
        nargs="*",
        required=True,
        help="Test results json files",
    )
    args = vars(parser.parse_args())
    create_charts(args["out"], args["results"])
